...
describe('BoardFormBuilderService', () => {
  ...
  describe('of', () => {
    ...
    it('should create a form control for each row', () => {
      ...
      expect(Object.keys(actual.controls).length).toBe(Object.keys(board).length);
    });

    it('should create a form control for each column', () => {
      const rowKeys = Object.keys(board);
      const [randomRowKey, randomColumns] = rowKeys.map(rowKey => [rowKey, board[rowKey]])[
        faker.random.number({ min: 0, max: rowKeys.length - 1 })
      ];
      const actual = service.of(board);

      expect(Object.keys((actual.get(randomRowKey) as FormGroup).controls).length).toBe(
        Object.keys(randomColumns).length
      );
    });
    ...
    it('should return a form group which value is equal ', () => {
      ...
      Object.keys(boardWithoutErrorMessages).forEach((rowKey: string) =>
        Object.values(boardWithoutErrorMessages[rowKey]).forEach((column: Column) => delete column.errorMessage)
      );
      const actual = service.of(board);
