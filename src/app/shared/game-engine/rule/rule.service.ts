...
export class RuleService {
  applyRules$: BehaviorSubject<Partial<GameState>> = new BehaviorSubject<Partial<GameState>>(null);
  ...
  rulesApplied$: Observable<GameState> = this.applyRules$.pipe(
    ...
    map(([newGameState, onlyToTheChangedColumnsOfActualState, toPlayer]: [GameState, GameState, Player]) => [
      setPlayerForBoardIn(newGameState, onlyToTheChangedColumnsOfActualState, toPlayer),
    ...
    map(([newState, actualState, boardDifferences]: [GameState, GameState, BoardDifferences]) =>
      applyRules(this.rules)(Object.freeze(newState), Object.freeze(actualState), boardDifferences)
    )
  );
